...
export function Header() {
  ...
  const authMenuItems = session?.user ? (
    <>
      <DropdownMenuItem disabled className="cursor-default">
        <span className="mr-2 text-base">🔥</span>
        <span>
          Streak: <strong>{session.user.streak || 0}</strong> days
        </span>
      </DropdownMenuItem>
      <DropdownMenuItem disabled className="cursor-default">
        <Trophy className="mr-2 h-4 w-4 text-amber-500" />
        <span>
          Score: <strong>{(session.user.totalScore || 0).toLocaleString()}</strong>
        </span>
      </DropdownMenuItem>
      <DropdownMenuSeparator />
    ...
    <>
      <DropdownMenuItem disabled className="cursor-default">
        <span className="mr-2 text-base">🔥</span>
        <span>
          Streak: <strong>{localStreak}</strong> days
        </span>
      </DropdownMenuItem>
      <DropdownMenuSeparator />
      ...
      <div className="flex items-center gap-3">
        ...
        {displayStreak > 0 && (
          <div className="hidden sm:flex items-center gap-1.5 bg-amber-50 dark:bg-amber-900/20 px-3 py-1 rounded-full">
            ...
            <span className="font-bold text-amber-600 dark:text-amber-400 tabular-nums">{displayStreak}</span>
          </div>
        )}
